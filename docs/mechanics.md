...
- ✅ **Sound: the music plays — and for weeks this line was wrong.** 771 `.wav` files,
  ...
  **The number that keeps this row honest is 555 of 771**, and it is measured rather
  than typed — `crates/l2-game/tests/audio_wiring.rs` drives the real paths and reads
  back what was actually opened. **543 of those 555 are the narrator**, 448 lord takes
  ...
  *"front end"* forever (`docs/decisions.md` C116) — then 11, then 555 when the message
  window arrived and the voice class came with it. Everything below this paragraph is
  ...
  **And the count that says what to do next is 24 of 134.** `docs/audio-triggers.md` is
  the audio equivalent of the input-arm audit: every place `Lords2.exe` asks for a sound,
  generated by `node tools/oracle/sounds.js` rather than typed. It was cheap — the
  original funnels every sound through **eight leaf functions**, so the denominator is a
  grep and not a reading of every screen. **The two counts disagree by a factor of twenty
  and both are right**: 16 of the 134 sites carry 543 of the 771 files, because
  `Msg_PlayVoice` is a table lookup and the other primitives are mostly constants. A site
  count weights every trigger equally and a player does not. The largest thing still
  missing is the two sample banks, 49 sites: the village's work, the marching army, the
  merchant's cart, every sword and every arrow.

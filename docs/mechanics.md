...
- ✅ **Sound: the music plays — and for weeks this line was wrong.** 771 `.wav` files,
  ...
  **0 of 771** from the day the layer landed until a player said he heard nothing —
  ...
  end"* forever. `docs/decisions.md` C116. Everything below this paragraph is
  ...
  **And the count that says what to do next is 7 of 134.** `docs/audio-triggers.md` is
  ...
  grep and not a reading of every screen. The largest single thing missing is the two
  sample banks, 49 sites: the village's work, the marching army, the merchant's cart,

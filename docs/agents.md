...
4. **The lead is the only writer of `docs/work.json`, on branch `pm/ledger`.** Agents report;
   ...
   46: order (the merge queue is read top to bottom) and its one-row-per-line shape,
...
66: the backlog and the numbers folded away — one line per item,
...
73: cite a correction, a path, or an `arms:`/`audio:` inventory entry,
...
budget, read-only but for the one file it writes), given the branch name and nothing else.
...
**The record.** Its last step is `docs/oracle-checks/<branch>.json`: `reviewed` (the commit
read), `date`, and one `{file, line, address, verdict, note}` per behaviour, verdict
`match` / `mismatch` / `departure` — so the project can say *which* behaviours were read
against the binary, not only that a check ran. `node tools/oracle/checked.js` joins every
record to the crates and prints per crate: cited lines (any line naming `0x00XXXXXX`,
`FUN_`, `DAT_` or `LAB_`), checked, stale (the line's text is not in the file at the
record's `reviewed` commit) and unchecked; `--json` feeds the one header line of
`work.js --html`, `--verify` is what `crates/l2-testkit/tests/oracle_checks.rs` runs.
The eight records dated 2026-09-14 carry `"source": "seeded from the conversation, not the
report"`: verdict level only, taken from the branch diffs, nothing re-read (C234).
...
A doc block is those lines in this order: the function,
...
213: only accepts paths git already knows,
...
223: that would sweep the index,
...
229: artefacts this way put all 1,891 straight back,
...
303: ## Never drive the real mouse or keyboard to test our engine
...
316: CI and fails on a duplicate number, naming both headings and the next free one;
...
321: It is still not worth serialising the log,
...
357: The first proposal was to make citations self-describing — write `C34 (secession-tie)`,
...
429: *identical*. Text is an opaque blit,
...
447: read is a check whose message is untested,
...
474: working. An accidental **fail-to-fail** is indistinguishable from a broken instrument,
...
513: That is C30 from the drawing side, not the save side,
...
542: `Grain_LabourEstimate` and stops.
543: harvest forecasts and the signed change the sidebar draws — were carried by nothing**,
...
634: *smaller* than the fix that was asked for,
...
784: collision will be between two branches that both cite the same file,
...
791: Three of these in two days,
...
* The stamp test now draws the page, copies it, draws the stamp **again** on the copy, and
  825: requires the two to be identical. Text is an opaque blit,
...
847: that **positional merging is correct for data and wrong for claims,
...
880: The union produced invalid JSON, the parse threw,
...
891: have regenerated `symbols.md` from it without complaint, both would have been green,
...
947: Everything above is abstract until it costs something. This is the one that nearly did,
...
1013: default — `c.farm_style = s.farm_style;`, forty-odd lines of them —
...
1034: ### The producer is protected and the consumer is not,
...
1038: to make the importer build its `County` with an exhaustive literal,
build.
...
1093: except the one that mattered: what was found was the *prologue of the industry branch*,
...
1142: count was wrong too (two, not eight),
...
1155: dropped, a count that read as success, a rename that quietly unhooked a table —
...
1159: It is the exact twin of the `ApplySymbols` tally that reported two failures for one,
...
1229: would have caught **none of them**. Worse,
...
3. **The two lists must agree, both ways.** Each handled gesture in a screen module carries a
   1280: one-line marker — `// arm: 0x004A8E0B` or `// arm: ours` —
...
4. **A per-screen percentage, printed by the census.** Not a gate: a number in `status.html` and
   ...
   1288: already does exactly this for other counts,
...
1324: So the totals in C61 are provisional until the corrected denominator lands,
...
1343: the coincidence and **declined to build the story on it**,
match unexplained.
...
* `corrections.js` checked that
  ...
  1387: still saying `C61`, which by then was a different correction entirely,
...
1394: The shape is one shape. **Every check we have is exhaustive over a set,
...
1427: This is the fourth stale-document mechanism catalogued here,
...
1652: reallocates the county twice and calls `Panel_Ration()` —

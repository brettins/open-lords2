...
| `0x004AB383` | `Battle_ReturnToCampaign(resume)` | verified | Applies a battle result to the campaign: charges the winner moves (+8, or all but one for a human owner), clears garrison and siege links, flips a captured county, and destroys the loser - except under autocalc, where an army left with 50 or more men only has its siege lifted. Diplomacy takes a 20-point hit. |
...
| `0x0046902A` | `County_PlaceBlacksmith(county)` | verified | Places the weapons industry site (record 2) on the plain tile of the county nearest one of its farm fields. The blacksmith is derived rather than authored, which is why every county has one. |
...
| `0x0043744F` | `Army_DisbandConfirm()` | verified | The 'Disband army?' yes-callback: Army_Disband, or network command 0x2E under g_deterministicBattle. |
...
| `0x004A7948` | `Army_GarrisonConfirm()` | verified | Yes-callback of the confirmation Army_Garrison stashes its arguments for: restores g_screenId from g_screenIdSaved and runs Army_GarrisonApply, or sends network command 0x34 under g_deterministicBattle. Its twin one function later does the same for a siege with command 0x35, and that pairing is how those two opcodes are identified. |
| `0x004A7DAF` | `Army_BeginSiegeConfirm()` | verified | Yes-callback for 'Besiege castle?': Siege_Link, or network command 0x35 under g_deterministicBattle. |
| `0x0043EDA0` | `Net_SendCommand(op, dest)` | verified | The deterministic-play command sender, and the reason every campaign action in the binary is written twice - once as a direct call and once as Net_SendCommand(op, 0) under g_deterministicBattle. Looks the opcode up in g_netCmdLength (0xFF is not a command and errors), calls g_netCmdWriters[op] to serialise the payload, stamps opcode, length and a sequence number that wraps 1..39 into a 0x104-byte slot at 0x00512FB0, and sends the packet once for eleven listed opcodes and three times - with flag bits 0x40 then 0x80 - for all the others. Two invariants close: all 112 entries of g_netCmdWriters are real function starts, and opcode 0x29's writer emits 1+4+1+2+2 bytes, exactly the 10 g_netCmdLength records for it. The campaign-map opcodes are 0x29 move order, 0x2C move-and-combine, 0x2E disband, 0x34 garrison, 0x35 begin siege, 0x3A toggle industry from the map. |
...
| `0x0057C924` | `g_battleLoser` | verified | The unit index of the defeated side, read by Battle_ReturnToCampaign to decide which army is destroyed and which is charged movement. |
...
| `0x004D5B90` | `g_netCmdLength` | verified | u8[112]: the payload length of each deterministic-play command, 0xFF for an opcode that is not one. Valid opcodes run 0x01..0x61; 0x00 and 0x62..0x6F are all 0xFF. Net_SendCommand rejects a 0xFF opcode outright. Opcode 0x29's 10 matches its writer's 1+4+1+2+2 exactly, which is what makes the table's reading a finding rather than a guess. |
...
| `0x0053FA24` | `g_county0_castleX` | verified | X of county 0's castle tile in the county record array (stride 0x300), written by County_FindCastleTile. battle-before.sav confirms it from the data side: county 2's pair is (30, 46), that tile carries plane-0 bit 0x80 with terrain 0x15, and unit slot 4 - the army whose garrisonCounty is 2 - stands exactly there. Distinct from the anchor at +0x6C/+0x6D, which is the county TOWN. |
| `0x0053FA25` | `g_county0_castleY` | verified | Y of county 0's castle tile, as above. |
| `0x0053FA20` | `g_county0_townTile` | verified | i32 byte offset into g_tiles of the TOP-LEFT tile of county 0's 2x2 county town; the anchor at +0x6C/+0x6D is that block's bottom-right corner. Map_Click centres the map on it before opening the village. The relation holds on all four occupied counties of battle-before.sav - (14,37)/(15,38), (30,49)/(31,50), (36,15)/(37,16), (48,41)/(49,42) - which is what makes County_FindDefendingArmy's asymmetric -2..+1 window read as 'the town block plus its one-tile ring'. |

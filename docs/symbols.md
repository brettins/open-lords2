...
| `0x00466C3C` | `Unit_TryEnterTile(tileOffset)` | verified | Classifies the target tile and returns a code: occupied -> Unit_EnterOccupiedTile; road (plane0 0x01) -> 3; castle (0x40) -> 5; settlement (0x80) -> 6; dwelling plot (0x10) -> 7; farm field (0x20) -> 8; otherwise 1. |
...
| `0x0041F98B` | `ScenarioList_Draw()` | verified | The five-row scenario list box, with the selected row inverted and a proportional scrollbar thumb whose three segments are computed from the scroll offset, 5, and the total. Row text is L2.eng group 101, the 60 map slot names, indexed through DAT_0050A460 so the list can be filtered rather than being a straight window onto the group. |

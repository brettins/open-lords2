...
| `0x00482F86` | `BattleMan_StateNone` | verified | Slot 0 of g_manStateTable: an empty stub, `ret`. State 0 is the do-nothing state, and BattleMan_StateMelee drops a figure to state 8 rather than 0, so nothing in the field battle reaches it. |
...
| `0x004D9C90` | `g_engineEdgeOrtho` | verified | Rows of three cell-offset deltas, indexed as [direction * 0x0C + i * 4]: the three cells a 3-wide siege engine sweeps as it steps. Cell_TryEnterEngineOrtho indexes it with directions 0, 2, 4 and 6 only, so the odd rows are dead; g_engineEdgeDiag is the odd-direction table. |
| `0x004D9CF0` | `g_engineEdgeDiag` | verified | The diagonal counterpart of g_engineEdgeOrtho, read by Cell_TryEnterEngineDiag. |

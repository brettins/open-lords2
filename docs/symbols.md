...
| `0x0057C924` | `g_battleLoser` | **misnamed** | **It holds the WINNER.** Two sites agree against the name: the autocalc sets it to B when A is the weaker side, and `Battle_ReturnToCampaign`'s `== g_battleArmyA` branch gives the county to A, charges A the winner's movement and destroys B. Read as named, an implementation destroys the winner. Rename to `g_battleWinner`; `docs/armies.md` §7 has the evidence. |

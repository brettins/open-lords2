...
| `0x004AA181` | `Army_Combine(into, from)` | verified | Merges one army into another when the combined men are under 1501 and at most one of them carries mercenaries (otherwise message 0xA7, L2.eng 167). Sums men, the seven troop counts and the three siege-engine records, and keeps the lower movesUsed. |
...
| `0x0044AE51` | `Territory_SecedeMinorBlocks()` | verified | For each realm in play, finds its most populous block in g_territoryBlocks and calls County_MakeIndependent on every county in each of its other blocks. Ties go to the lowest block index, because the comparison is <=. Only the local player is told: message 0x7F when exactly one county went, 0x80 when more. An AI loses its outlying counties in silence. |
...
| `0x00428C42` | `Ale_Apply(crowns, county)` | verified | The ale happiness rule, and the only writer of county +0x219. Its ladder is on crowns spent against population/10: one point per tenth of the population's worth of crowns, five at most, then clamped again to 5 minus +0x219 - the total ale has ever given this county. The gain is added to happiness, to +0x219, and to the shownAle display field. Nothing in the binary resets +0x219, so the cap is for the life of the game. Note the argument is the money, not the barrels: Merchant_Trade passes buyPrice * qty. |
...
| `0x0042847C` | `Merchant_ResetStall()` | verified | Reloads the live stall at 0x005532A0 - 16 goods of 16 bytes - with the base prices from g_goodsPrice and zeroes each good's second word. Sheep and wool are copied in with their price of zero like everything else, so the stall carries rows for two goods Merchant_Trade has no branch for. |
...
| `0x0049A990` | `Ai_ManageFarmsAll()` | verified | The first thing Season_Advance does, before Rand_Advance: Ai_ManageCountyFarms for every realm that is in play and not human. It runs ahead of the whole economy, so an AI's fields and labour split are already this season's when tax, rations and industry read them. |
...
| `0x0049B2A3` | `Mp_DropDepartedPlayers()` | verified | Season pass 1. For each realm that is marked human, has no lord, and whose slot in the 6 x 0x2C connection table at 0x00553D50 has gone to zero, calls Realm_Eliminate, decrements the player count at 0x00553F98, and sets lord = 6 - the knocked-out value. [V] Such a realm still holds its counties, so Realm_Eliminate takes the second branch and sends group 258, "This player has left the game", which is the case this pass exists to produce. The gate is g_deterministicBattle, whose name is only inferred and is probably a network-game flag; that reading is not settled here. |

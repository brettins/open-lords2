...
We **halt** instead, for the reason stated above: a resynchronised session hides the bug
that caused the divergence, and the original's own shipped `status.txt` counts
`Net play divergance count` and `Net play wipe outs` as routine telemetry — evidence that
it diverged often enough to need a policy. Our position is unchanged by this; what
changes is that it is now a considered disagreement with a known design rather than an
assumption.

...
* **Nothing anywhere writes surface 4 outside the classifier**, so 4 cannot be *"what a breach
  leaves behind"*, which is what `crates/l2-sim` called it. `docs/decisions.md` `CNEW-surfaces`.
...
That third property is the one with teeth, and `docs/decisions.md` `CNEW-lineclear` records what
...
> `docs/decisions.md` `CNEW-smash`. A one-cell breach is a funnel an army cannot press through, and
...
`docs/decisions.md` `CNEW-0055307c`.
...
> `docs/decisions.md` `CNEW-approach`.
...
* **`Wall_Smash` (`0x0049694F`)** — the radius-4 square both wall-hitting states run on their
  ...
  ever with its gate open. That was ours and it is `docs/decisions.md` `CNEW-surfaces`.
...
and they arrive when the holes do. `docs/decisions.md` `CNEW-defenceposts`.

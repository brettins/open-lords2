...
turned up four defects of our own, which are `CNEW-moat-unreachable`,
`CNEW-keep-unreachable`, `CNEW-drawbridge-not-engines` and `CNEW-defender-choice` in
`docs/decisions.md`.
...
`g_foundTileX`/`Y` are left at `(0, 0x50)`. `docs/bugs.md` `CNEW-drawbridge-scan`.

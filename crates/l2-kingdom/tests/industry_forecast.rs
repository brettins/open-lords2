...
/// **`BNEW-forecast`: the row is multiplied by the ramp at the county's whole
...
/// the row **understates**. `docs/bugs.md` BNEW-forecast called it optimistic;
...
fn the_row_is_drawn_at_the_ramp_a_whole_population_would_earn_and_understates_because_of_it() {
    ...
    assert_eq!(
        ...
        "county {id}: the row draws the whole-population ramp's number ({drawn}), which is \
         `docs/bugs.md` BNEW-forecast, and not the workforce's own ({honest})"
    );
}

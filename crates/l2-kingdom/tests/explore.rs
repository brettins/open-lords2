//! **The fog of war's writers**, each driven through the rule that owns it.
//!
//! `l2_kingdom::explore` has the table: every writer of the original's seen bit,
//! with its address. These are the four of them this crate runs — an army
//! raised, an army walking, a county taken — and the save that has to carry the
//! result. The fifth, the new game's start county, is `l2-scenario`'s and is
//! tested there, beside the England fixture that checks all of them against
//! bits the original wrote.
//!
//! **Ablations, run on this branch and stated so the next person need not
//! guess.** Each is the one line the test names, deleted:
//!
...
/// A kingdom with county 1 on every tile, owned by realm 1, anchored at (20, 20)
/// and big enough to levy from.
...
/// `realm == 0` raises an ownerless militia, owner 6, and **6 is never
/// `g_localPlayer`**, so nobody sees anything.
#[test]
...
/// A unit of `kind` for realm 1 at (10, 10), ordered ten tiles east along a
/// road and walked by `tick_units` until it stops.
...
///
/// Eleven centres along row 10, from x 10 to x 20, each seeing six either side:
...
/// **`kind == 1`.** A merchant, a transport or a mob walks the same road and
/// lifts no fog for anybody — even one given to realm 1 here, which a real
/// merchant never is.
#[test]
...
/// The plane is simulation-written, cannot be rebuilt, and so is carried —
/// every bit of every realm, through our own save format.
#[test]

...
/// |---|---|---|
/// | `FUN_0040d6ad#1`, `FUN_0040d7b8#1` | the two arrows of a slider widget whose hit-tester `FUN_0040D3F5` has **no caller**: zero rel32 calls, zero jumps and zero absolute references in the image, against 36 rel32 callers for `Widget_Test` as the control | `DNEW-slider-widget` |
...
fn the_dead_triggers_are_the_three_on_the_bug_list() {
    ...
    for (id, entry) in
        [("slider arrows", "DNEW-slider-widget"), ("Battle_PauseButton#1", "**D38**")]
    {

...
fn main() {
    ...
    // recompiles `l2-game`: 5 to 7 seconds against 0.19 for a no-op build.**
    //
    ...
    // **Always rerun.** The two `rerun-if-changed` lines below replace cargo's
    // default of "any file in this package", and `make_room_for_the_link` has to
    // run before *every* link or it protects only the builds that happen to
    // follow a commit — an edit to `l2-kingdom` relinks this binary without
    // touching `HEAD` or `index`. Naming a path that does not exist is how a
...
///
/// Windows will not let a link step overwrite a running executable, so a player
/// with the game open turns every build in the workspace red — not only
/// `cargo build`, but `cargo test --workspace`, which links this same binary.
...
/// than assumed: overwriting a running exe fails with *"Device or resource
/// busy"*, and renaming the same file succeeds, after which a fresh exe writes
/// to the original path while the old process keeps running out of the renamed
/// image. So the fix is to get the old file out of the way, not to give the new
...
/// policy is needed and none is implemented**: every build sweeps every
/// `l2-game.old-*.exe`, and the only ones that survive are the ones still
/// running, which are precisely the ones that must. They are collected on the
/// next build after the player quits. If the sweep ever leaves more than a
/// handful behind, that is reported rather than silently accumulated — it would
...
/// On failure the build proceeds and cargo reports the link error it would have
/// reported anyway.
#[cfg(windows)]
fn make_room_for_the_link() {
    ...
    if let Ok(entries) = fs::read_dir(&profile) {
        for e in entries.flatten() {
            ...
            if name.starts_with("l2-game.old-") && name.ends_with(".exe") {
    ...
    let stamp = std::time::SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    let aside = profile.join(format!("l2-game.old-{stamp}.exe"));

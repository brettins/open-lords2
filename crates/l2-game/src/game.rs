...
pub struct Quirks {
    // No presentation quirk has landed on `main` yet. The first is the county
    // name's parchment emboss (`docs/bugs.md` B64), on another branch; when it
    // merges it is one `pub bool` here and one row in `PRESENTATION`, and the
    // catalogue check demands both.
}
...
impl Quirks {
    ...
    pub fn set_fixed(&mut self, field: &str, fixed: bool) {
        let _ = fixed;
        debug_assert!(
            !PRESENTATION.iter().any(|(f, _)| *f == field),
            "{field} is in PRESENTATION and has no arm in Quirks::set_fixed"
        );
    }

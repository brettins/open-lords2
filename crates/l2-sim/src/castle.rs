...
//! `Battlefield_BuildCastle` (`0x0047C4BA`), `FUN_0047CEC1` and the six
//! surface passes of `FUN_0047E230`.
//!
...
//! * the **structure** layer is markers, not tiles: `FUN_0047CEC1` walks it for
...
pub struct CastleSheet {
    ...
    pub frames: Vec<u8>,
    /// Directory `+0x1C` — the marker layer `FUN_0047CEC1` walks.
...
pub fn build(level: u8, sheet: &CastleSheet) -> Battlefield {
    ...
    // `FUN_0047DCCE` runs the 49-entry water table over the whole plane; ours
    ...
    for i in 0..CELLS {
        ...
        if (ESCAPE_KEEP_FRAME..=ESCAPE_GROUND).contains(&b) {
            ...
            match b {
                ...
                // `FUN_0047DCCE`: the moat. It also zeroes the approach score,
...
/// **The surface classifier** — `FUN_0047E230` (`0x0047E230`), six flood
...
/// |---|---|
/// | `FUN_0047E263` | the keep (6) outward over surface `0x0E` and over anything above elevation 3 |
/// | `FUN_0047E387` | the bailey (5) over low ground beside the keep door, a 7 or another 5 |
/// | `FUN_0047E52D` | the rampart walk (4) over raised ground beside the bailey or another walk |
/// | `FUN_0047E668` | the apron (3) over flat ground within **eight** neighbours of a walk or another apron |
/// | `FUN_0047E7B2` | the open field (1), seeded at cells `0` and `0x18B0` — `(0, 0)` and `(0, 79)` |
/// | `FUN_0047E926` | what is left: 3 beside water, 6 beside the keep |
///
...
/// **The positional tables the structure layer carries** — `FUN_0047CEC1`
...
/// **Walk the structure layer** — `FUN_0047CEC1`.
///
...
///   clamped to `0..=11` (`FUN_0047D6B0`);
/// * side 0's other kinds — a four-bit index, `buf[+0x50 ..= +0x53]` each
///   contributing `8, 4, 2, 1` when it holds **7** (`FUN_0047D5EC`);
...
pub fn tables(sheet: &CastleSheet) -> CastleTables {
    ...
    // `FUN_0047D5EC` and `FUN_0047D4B8`: the row below the marker, read as bits
    // and then cleared.
    ...
    for y in 0..DIM {
        for x in 0..DIM {
            ...
            // `FUN_0047D474`: take the kind and clear it, and `+3` with it.
            let kind = b[i + 1];
            b[i + 1] = 0;
            ...
            if marker == MARKER_SIDE0 {
                if kind == MARKER_SIDE0 {
                    // `FUN_0047D6B0`, and the y is the marker's own row.
...
/// this is the table `FUN_0047CEC1` writes.
///

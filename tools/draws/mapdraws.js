...
// **The reproduced count, read out of `docs/draws-map.md`'s area table.**
//
// The listing below says how many calls each painter makes; how many of them WE
// make is a judgement per area, and it lives in that table. Totalling it here
// rather than quoting a number beside it means the two cannot drift — which is
// the whole reason this file exists rather than a paragraph saying 139.
function ours() {
  const md = fs.readFileSync(
    require('path').join(__dirname, '..', '..', 'docs', 'draws-map.md'),
    'utf8',
  );
  let live = 0;
  let mine = 0;
  ...
  for (const line of md.split('\n')) {
    // `| area | live | ours | missing | …` — the body rows only.
  ...
  if (rows < 4) {
    console.error('mapdraws: the area table in docs/draws-map.md has ' + rows + ' rows, expected 4+');
    process.exit(1);
  }
  return { live, mine };
}
...
// **The reproduced fraction, and it is checked against the listing above.**
//
// `docs/plan.md` §0 quotes this as the instrument for its third falsification
// row — the one that read "instrument: none" from the day it was written — so
// it is derived twice and required to agree: the listing above counts every
// live call, the area table in `docs/draws-map.md` accounts for all of them
// and says how many are ours. A reproduced fraction over the wrong
// denominator is exactly the failure this audit exists to measure.
const o = ours();
if (o.live !== live) {
  console.error(
    `mapdraws: the listing counts ${live} live calls and the area table in ` +
      `docs/draws-map.md accounts for ${o.live}. One of them is wrong.`,
  );
  process.exit(1);
}
const pct = Math.round((o.mine / o.live) * 100);
console.log(`reproduced ${o.mine} of ${o.live} live (${pct}%)`);

if (process.argv.includes('--figures')) {
  console.log(JSON.stringify({ total, live, debug, dead, ours: o.mine, pct }));
}
